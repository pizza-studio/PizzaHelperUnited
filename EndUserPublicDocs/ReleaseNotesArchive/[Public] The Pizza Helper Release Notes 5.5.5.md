...
- 新增了对《星穹铁道》3.5 的新角色与新武器的支持。另注：本软件今后对在原神 v6.0 的支持可能会略有日程上的延迟。
- 抽卡记录管理器（GRM）：正式宣布对 UIGF v4.1 资料交换标准的支持。
- GRM：现对常驻命中率（SHR）的显示做出改良，新增自动可信度检测功能，对样本量不足显示警告文本，并对高于51%的常驻命中率施加中等可信度上限，以降低法律风险。
...
- 新增了對《星穹鐵道》3.5 的新角色與新武器的支援。另注：敝軟體今後對在原神 v6.0 的支援可能會略有日程上的延遲。
- 抽卡記錄管理器（GRM）：正式宣佈對 UIGF v4.1 資料交換標準的支援。
- GRM：現對常駐命中率（SHR）的顯示做出改良，新增自動可信度檢測功能，對樣本量不足顯示警告文本，並對高於51%的常駐命中率施加中等可信度上限，以降低法律風險。
...
- Added support for new characters and weapons introduced in Houkai: Star Rail v3.5. Note: The support of Genshin Impact v6.0 in this app will likely to be delayed a little.
...
- 当アプリはこのアップデートにて『スターレイル』v3.5に対応しました。補記：今後、当アプリにおける「原神 v6.0」への対応は少し遅れる可能性があります。
- ガチャ記録マネージャー（GRM）：UIGF v4.1 データ交換標準のサポートを正式発表。
- GRM：通常命中率（SHR）の表示を改良し、自動信頼性検出機能を追加。サンプル数が不足している場合の警告テキスト表示、51%を超える通常命中率に対する中程度の信頼度上限設定により、法的リスクを軽減。
...
(ВНИМАНИЕ: Приложение может столкнуться с проблемами команды разработки. В худшем случае, оно может быть удалено из App Store и переиздано под новым именем и App Bundle ID, что сделает невозможным доступ к старым данным из-за использования Group Containers и поддержки macOS. Пользователям придётся восстанавливать данные из резервных копий. Рекомендуется регулярно создавать резервные копии. Окончательное решение будет принято до конца ноября 2025 года. Переизданное приложение может быть временно недоступно в материковом Китае из-за ICP-регистрации.)

- Добавлена поддержка новых персонажей и оружия из Houkai: Star Rail v3.5. Поддержка Genshin Impact v6.0 может быть отложена.
- Менеджер записей Gacha (GRM): Теперь поддерживает стандарт UIGF v4.1.
- GRM: Улучшен показатель стандартного уровня попаданий (SHR) с автоматическим определением надёжности, предупреждениями о малом объёме выборки и ограничением доверия для SHR выше 51% для снижения рисков.
- GRM: Оптимизирована производительность UI с помощью устранения дребезга для вертикальных столбчатых диаграмм.
- GRM: Сокращено расстояние между элементами в горизонтальных столбчатых диаграммах.
- GRM: Термин «Средн. попыток (лимитированная 5-звёздочная)» заменён на «Средн. попыток (не-стандартная 5-звёздочная)».
- GRM: Изменён макет основного экрана «владельца гача» для экономии места.
- GRM: Введён новый термин «владелец гача (ガチャ主)».
- GRM: При устаревании кэша GachaMetaDB приложение сначала использует встроенные данные, а затем пытается обновиться онлайн.
- Переключатель языка теперь доступен на OS 21 (iOS 14 / macOS 11) с возможностью отмены.
- Добавлены недостающие переводы для экрана OOBE.
- Изменён макет меню типов испытаний в отчёте о боях.
- В EULA добавлен отказ от ответственности за статистику, основанную на поддельных данных.
- Исправлены ошибки выравнивания в таймере выносливости.
- Для уменьшения размера приложения, встроенные именные карточки Genshin уменьшены до 210x100. При необходимости, будут загружаться онлайн-версии в высоком разрешении.
- Японский интерфейс был скорректирован с помощью GitHub Copilot для улучшения читабельности.
- Некоторые части исходного кода, включая EnkaKit, теперь выпускаются под лицензией MIT.
