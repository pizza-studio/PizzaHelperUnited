// 《拿铁小助手》v5.8.9 的更新内容简述：

- 更新了《星穹铁道》v4.4 的角色证件照。
- 引入下述更动，借此改良与未来系统的相容性：
	- 重写了后台任务调度系统。
	- 调整了详情页签顶端的本机帐号切换选单按钮的位置。
	- 重写了忙碌指示动画。
...
// 《拿鐵小助手》v5.8.9 的更新內容簡述：

- 更新了《星穹鐵道》v4.4 的角色證件照。
- 引入下述更動，借此改良與未來系統的相容性：
	- 重寫了後台任務調度系統。
	- 調整了詳情頁簽頂端的本機帳號切換選單按鈕的位置。
	- 重寫了忙碌指示動畫。
...
// Major changes introduced in The Latte Helper v5.8.9:

- Added new ID photos for new characters introduced in Star Rail v4.4.
- Following changes are introduced for better compatibility with future OS releases:
	- Refactored the task management module.
	- Details Tab: Changed the position of locale profile switcher dropdown menu button.
	- Refactored the implementation of ring-type infinite progress indicator animation.
...
- 『スターレイル』v4.4 で追加された新キャラクターの身分証明写真を追加しました。
- 将来の OS リリースとの互換性向上のため、以下の変更を導入しました：
	- バックグラウンドタスクスケジューリングシステムをリファクタリングしました。
	- 詳細タブ：ローカルプロファイル切り替えドロップダウンメニューボタンの位置を変更しました。
	- リング型無限進捗インジケーターアニメーションの実装をリファクタリングしました。
...
// Основные изменения в «Латте помощник» v5.8.9:

- Добавлены новые фотографии персонажей, представленных в Star Rail v4.4.
- Внесены следующие изменения для улучшения совместимости с будущими версиями ОС:
	- Переработан модуль управления фоновыми задачами.
	- Вкладка «Подробности»: изменено расположение кнопки выпадающего меню переключения локального профиля.
	- Переработана реализация анимации кольцевого индикатора прогресса.
