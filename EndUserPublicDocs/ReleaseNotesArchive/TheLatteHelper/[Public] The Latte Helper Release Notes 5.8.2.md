// 《拿铁小助手》v5.8.2 的更新内容简述：

- 紧急修复了与米游社账号 QRCode 登入有关的问题。
- （《星穹铁道》v4.3 的支援被顺延至下一个版本。）
...
// 《拿鐵小助手》v5.8.2 的更新內容簡述：

- 緊急修復了與米遊社帳號 QRCode 登入有關的問題。
- （《星穹鐵道》v4.3 的支援被順延至下一個版本。）
...
// Major changes introduced in The Latte Helper v5.8.2:

- Emergency fix: Resolved an issue related to QR Code login with Miyoushe accounts.
- (Star Rail v4.3 support has been deferred to the next version.)
...
- 米遊社アカウントにおけるQRコードログインに関する問題を緊急修正しました。
- （『スターレイル』v4.3 への対応は次バージョンに順延されました。）
...
// Основные изменения в «Латте помощник» v5.8.2:

- Срочное исправление: решена проблема, связанная со входом в учётную запись Miyoushe через QR-код.
- (Поддержка Star Rail v4.3 перенесена на следующую версию.)
