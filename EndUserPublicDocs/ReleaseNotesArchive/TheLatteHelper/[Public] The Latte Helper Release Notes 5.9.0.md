// 《拿铁小助手》v5.9.0 的更新内容简述：

- 本地账号管理画面现在不会自动触发「对无效的本地账号的清理」的操作。在侦测到有这种本地账号的时候，本地账号管理画面现在会单独显示一个按钮、允许使用者手动触发清理操作。
- 修复了本地账号管理画面「无法删除本地账号」等一系列「资料修改不生效」的故障。
...
// 《拿鐵小助手》v5.9.0 的更新內容簡述：

- 本機帳號管理畫面現在不會自動觸發「對無效的本機帳號的清理」的操作。在偵測到有這種本機帳號的時候，本機帳號管理畫面現在會單獨顯示一個按鈕、允許使用者手動觸發清理操作。
- 修復了本機帳號管理畫面「無法刪除本機帳號」等一系列「資料修改不生效」的故障。
...
// Major changes introduced in The Latte Helper v5.9.0:

- The local profile manager no longer automatically triggers the cleanup of invalid local profiles. When such profiles are detected, the local profile manager now displays a dedicated button allowing users to manually trigger the cleanup operation.
- Fixed a series of bugs in the local profile management screen where data modifications (including profile deletion) were not taking effect.
...
- プロファイル管理画面は、無効なプロファイルの自動クリーンアップを実行しなくなりました。このようなプロファイルが検出された場合、プロファイル管理画面に専用ボタンが表示され、ユーザーが手動でクリーンアップ操作を実行できるようになりました。
- プロファイル管理画面で、プロファイル削除を含む一連の「データ変更が反映されない」不具合を修正しました。
...
// Основные изменения в «Латте помощник» v5.9.0:

- Экран управления локальными профилями больше не запускает автоматическую очистку недействительных локальных профилей. При обнаружении таких профилей на экране управления локальными профилями теперь отображается специальная кнопка, позволяющая пользователю вручную запустить операцию очистки.
- Исправлен ряд ошибок на экране управления локальными профилями, из-за которых изменения данных (включая удаление профилей) не вступали в силу.
