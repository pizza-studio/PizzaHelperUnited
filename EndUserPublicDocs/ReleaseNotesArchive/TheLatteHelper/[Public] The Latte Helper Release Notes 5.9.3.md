// 《拿铁小助手》v5.9.3 的更新内容简述：

- 新增对《星穹铁道》v4.5 的新角色与新武器装备的支持。新角色（及其命途）：罗缤•晴歌（记忆）、卡卡瓦夏•戏浪（欢愉）。
...
// 《拿鐵小助手》v5.9.3 的更新內容簡述：

- 新增對《星穹鐵道》v4.5 的新角色與新武器裝備的支持。新角色（及其命途）：羅繽•晴歌（記憶）、卡卡瓦夏•戲浪（歡愉）。
...
// Major changes introduced in The Latte Helper v5.9.3:

- Added support for new characters and new weapons introduced in Star Rail v4.5. New characters (and their life-paths) are: Robin • Summeretto (Remembrance) and Kakavasha • Waveflair (Elation).
...
- 《スターレイル》v4.5 で追加された新キャラクターおよび新武器への対応を追加しました。新キャラクター（とその運命）：ロビン・夏空の歌（記憶）、カカワーシャ・波と戯れる夏（愉悦）。
...
// Основные изменения в «Латте помощник» v5.9.3:

- Добавлена поддержка новых персонажей и нового оружия, добавленных в «Star Rail» v4.5. Новые персонажи (и их Пути): Робин: Яснопея (Память), Какавача: Прибой (Радость).
