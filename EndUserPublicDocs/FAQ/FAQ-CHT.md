...
若要匯入 Snap Hutao 難民檔案，請在抽卡記錄管理器內選擇 UIGFv4 格式並載入 Snap Hutao 的 `Userdata.db` 資料庫（由 Entity Framework Core 生成的 SQLite 資料庫）。App 會讀取此資料庫並將全部抽卡記錄轉為 UIGFv4 格式後匯入。披薩小助手無法繼承 Snap Hutao 的本機帳號資料，需要您自行重新設定您的本機帳號。

開發者們無權對米哈遊的決策做背書。目前可以講的是：從 App Store / TestFlight 安裝的披薩小助手並無任何涉及到對米哈遊任何一款電玩的記憶體注入之行為。Apple 不會允許此類行為，披薩小助手也從未實作過會需要去注入記憶體的功能。

#### Q: 我是舊版原神披薩小助手（Pizza Helper 4 Genshin）（截至 v4 版）的使用者，且舊裝置無法安裝 iOS 17+。請問在跨裝置遷移資料時該注意什麼？

如果您的新舊裝置都有藉由相同 Apple ID 開啟 iCloud Drive 且沒有將其空間用盡的話，舊版的抽卡記錄理論上是可以通過 iCloud 遷移的。然而，誰也無法保證這個過程會不會出現變數。這裡有手動遷移方法：請在舊版裝置上更新披薩小助手到 App Store 最新版，然後根據介面提示匯出披薩難民資料（Pizza Refugee Data）。之後，在新裝置上運行新版的披薩小助手：在本機帳號管理器內讀入的話，會匯入本機帳號資料；在抽卡記錄管理器內選 UIGFv4 格式匯入的話，會匯入抽卡記錄資料。
