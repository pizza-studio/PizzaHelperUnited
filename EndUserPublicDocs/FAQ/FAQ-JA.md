...
**米遊社アカウントへのログインはQRコードスキャンを通じて行われ、パスワードの入力は不要です**。

HoYoLABアカウントの場合、**iOSデバイスではシステム提供のセキュアな入力キーボードがパスワードの入力を取得し、macOSではハードウェアレベルのSecureEventInput APIが直接物理キーボードの入力を取得します**。**両方の入力はWKWebViewを通じてHoYoLABのログインページに直接渡されます**。その後、サーバーはデバイス固有のクッキーを生成し、このアプリは使用者の「情報のニーズ」に応じてそれを利用します。**この過程は他の誰にも（ピザ助手/ラテ助手アプリにも）傍受されません**。Appleのセキュリティ対策に感謝します。

ラテ助手 v5.7.7以降、HoYoLABアカウントはSign-in-with-Appleを使って、HoYoLABのログイン情報を入力せずにサインインするという選択肢が追加されました。
  - [原理] この実装はWKUIDelegateプロトコルに関連するWKWebViewのいくつかの準拠処理を適切に設定するだけで、Apple IDのログイン資格情報に触れません。また、この実装のコード変更では、`apple.com`ドメインを含むいかなる追加ドメインもWKWebView設定オプションに明記する必要がありません。しかし、サードパーティ製ソフトウェアはCognosphereの特別な許可および技術協力がない限りFIDO Passkeyによるパスワード不要ログイン機能を実装できません。したがってこの機能でApple IDにログインする場合はパスワードを使用する必要があります。セキュリティにお不安があれば、Appleが推奨するApple IDのセキュリティ設定（2要素認証など）を有効にしてください。
  - [ご注意] Apple IDでHoYoLABアカウントにサインインする前に、既存のHoYoLABアカウントとApple IDを必ずリンクしてください。このリンクはiOS版の『原神』『星穹鉄道』『絶区零』ゲーム内で行えます。リンクを完了する前にApple IDでサインインすると、HoYoVerseシステムが新しい空のHoYoLABアカウントを自動作成し、Apple IDをそのアカウントに強制的に紐付けます。そのような場合、新たに作成されたアカウントの削除クールダウン期間（約30日）を経てから元のアカウントに再リンクする必要があります。
  - なお、本アプリはGoogle、Facebook、Twitter（X）などのソーシャルメディアを介したHoYoLABアカウントログインを技術的にサポートしていません。
...
#### Q: 『崩スタピザ助手』の使用者です。この新しい『ピザ助手/ラテ助手（無印）』に移行する際に注意すべきことはございますか？

1. **データ同期**：新しい『統一ピザ助手/ラテ助手エンジン』で稼働している『ピザ助手/ラテ助手（無印）』は、同じApple IDでiCloudを通じて『崩スタピザ助手』のクラウドデータを読み取ることができますが、データ同期には時間がかかる場合があり、接続状態などの要因によって影響を受けることがございます。より確実な移行を行うためには、『崩スタピザ助手』からローカルアカウントをJSONファイル（プロファイル統括パッケージ）として書き出し、『ピザ助手/ラテ助手（無印）』に読み込むすることをお勧めします。ガチャ記録も同様の方法で移行できます（UIGF v4形式に準拠したファイル交換を通じて）。
...
当該サイトが提供するExcel形式のガチャ記録フォーマットには、UIGFv4規定に従った重要な情報欄が不足しているため、この前「原神ピザ助手」にはUIGFv4対応を実装した際に、Paimon.moeの独自Excelフォーマットのサポート機能を完全に放棄しました。この機能は現在の「ピザ助手/ラテ助手（無印）」にも実装されていません。当該サイトのガチャ記録データをUIGFv4標準に対応するアプリに読み込む必要がある場合は、Paimon.moeには「UIGFv4で記録を書き出す機能を実装なさる」よう、ご要求くださいませ。
