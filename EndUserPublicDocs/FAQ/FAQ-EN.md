...
To import Snap Hutao refugee data, please choose UIGFv4 format in the Gacha Record Manager and load Snap Hutao's `Userdata.db` database (a SQLite database generated by Entity Framework Core). The app reads the database and converts all gacha records into UIGFv4 format for import. The Pizza Helper cannot inherit Snap Hutao's local profile configuration; you need to reconfigure your local profiles manually.

The developers cannot endorse miHoYo's decisions, but to clarify: the App Store and TestFlight releaases of this app never perform any memory injection into miHoYo games; Apple would not allow such behavior. The Pizza Helper has never implemented features that require memory injection.
