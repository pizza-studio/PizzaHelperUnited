...
// MARK: ExpeditionsView.EachExpeditionView

@available(watchOS, unavailable)
extension ExpeditionsView {
    @available(watchOS, unavailable)
    private struct EachExpeditionView: View {
        ...
        private func pilotsView() -> some View {
            ...
            let shouldShrinkIconPaneWidth: Bool = layout == .tinyWithShrinkedIconSpaces && expedition.game != .starRail
            GeometryReader { g in
...
private func prepareExpeditionsView4Preview(
    ...
    layout: ExpeditionsView.Layout = .normal,
    max4AllowedToDisplay: Bool = false
)
    -> ExpeditionsView {
