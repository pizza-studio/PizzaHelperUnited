...
// MARK: - RealTimeNoteCardView

@available(iOS 17.0, macCatalyst 17.0, *)
struct RealTimeNoteCardView: View {
    // MARK: Lifecycle
    ...
    fileprivate static func renderRemainingTimeStringAsMultiline(_ targetTime: Date) -> String? {
        let timeIntervalSinceNow = TimeInterval.sinceNow(to: targetTime)
        let etaString = dateComponentsFormatter.string(from: timeIntervalSinceNow)
        guard let etaString else { return nil }
        return """
        \(etaString)
        \(dateFormatter.string(from: targetTime))
        """
    }
...
// MARK: RealTimeNoteCardView.NoteCardView4GI

@available(iOS 17.0, macCatalyst 17.0, *)
extension RealTimeNoteCardView {
    private struct NoteCardView4GI: View, ExpeditionViewSuppliable {
        // MARK: Lifecycle
            ...
            VStack {
                ...
                HStack(spacing: 10) {
                    ...
                    HStack(alignment: .lastTextBaseline, spacing: 0) {
                        ...
                        let nestedString = renderRemainingTimeStringAsMultiline(fullTime)
                        if fullTime > Date(), let nestedString {
            ...
            do {
                ...
                HStack {
                    ...
                    if homeCoin.fullTime > Date() {
                        ...
                        Spacer()
                        let nestedString = renderRemainingTimeStringAsMultiline(homeCoin.fullTime)
                        if let nestedString {
                            Text(verbatim: nestedString)
                                .multilineTextAlignment(.trailing)
                                .font(.caption2)
                ...
                HStack {
                    ...
                    case false:
                        if paraTransIntel.remainingDays > 0 {
                            ...
                            let fullTime = paraTransIntel.recoveryTime
                            let nestedString = renderRemainingTimeStringAsMultiline(fullTime)
                            if fullTime > Date(), let nestedString {
                                Text(verbatim: nestedString)
                                    .multilineTextAlignment(.trailing)
...
// MARK: RealTimeNoteCardView.NoteCardView4HSR
...
// MARK: RealTimeNoteCardView.NoteCardView4ZZZ

@available(iOS 17.0, macCatalyst 17.0, *)
extension RealTimeNoteCardView {
    private struct NoteCardView4ZZZ: View {
        // MARK: Lifecycle
                ...
                VStack {
                    if let timeOnFinish {
                        Text(timeOnFinish, style: .relative)
                            .multilineTextAlignment(.leading)
                            .frame(maxWidth: .infinity, alignment: .leading)
                        Text(RealTimeNoteCardView.dateFormatter.string(from: timeOnFinish))
                            .multilineTextAlignment(.leading)
                            .frame(maxWidth: .infinity, alignment: .leading)
                    }
                }
