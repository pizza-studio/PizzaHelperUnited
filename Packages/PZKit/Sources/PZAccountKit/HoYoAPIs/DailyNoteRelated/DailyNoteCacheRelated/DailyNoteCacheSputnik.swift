...
public struct DailyNoteCacheSputnik<T: DailyNoteProtocol> {
    ...
    public static func getCache(uidWithGame: String) -> T? {
        ...
        case let decodedHSR as Note4HSR:
            return decodedHSR.replacingBenchmarkTime(package.cachedTime) as? T
        case let decodedZZZ as Note4ZZZ:
            return decodedZZZ.replacingBenchmarkTime(package.cachedTime) as? T
